    ...
    <div class="column full-width">
      ...
      <div class="col q-my-md">
        <q-markup-table class="full-height text-center">
          ...
          <tbody>
            <tr v-for="(position, id) of positions" :key="id">
              <td>
                <q-checkbox class="q-ma-none" v-model="selection" :val="id" size="xs"></q-checkbox>
              </td>
              <td>{{getDepartment(position.departmentId).name}}</td>
              <td>{{position.name}}</td>
            </tr>
          </tbody>
        </q-markup-table>
...
export default {
  ...
  methods: {
    ...mapActions('positions', ['addUpdate', 'delete']),
    add() {
      this.resetData()
    ...
    edit() {
      this.mode = 'Edit'
      this.single = { ...this.positions[this.selection[0]] }
      this.selectedDepartment = {
        label: this.getDepartment(this.single.departmentId).name,
        value: this.single.departmentId
      }
      this.showSingle = true
    },
    save() {
  ...
  computed: {
    ...mapGetters({ positions: 'positions/getAll' }),
    ...mapGetters({ getDepartment: 'departments/getById' }),
    ...mapGetters({ departments: 'departments/getOptionsList' })
  },
  watch: {
    selectAll(val) {
      if (val) {
        this.selection = Object.keys(this.positions)
      } else {
        this.selection = []
      }
    }
